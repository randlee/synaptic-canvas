...
- Do not create artifacts during planning; only write/update a plan file.
- Plan status progression: **Preliminary → Proposed → [Review Gate] → Approved**
- **Review Gate**: Before marking Approved, all three review agents must pass:
  - `skill-architecture-review` (two-tier pattern, contracts)
  - `skill-implementation-review` (fenced JSON, hooks, security)
  - `skill-metadata-storage-review` (frontmatter, storage)
...
| Review gate failed | One or more review agents returned errors | Address blocking issues listed in review output, then re-run review gate |
