...
after each merge. As a stack they cost n CI runs and land atomically. Goal state: every PR
correctly based, mergeable, CI running on every layer.
...
cleanly. A **risky** conflict stops it: the rebase stays paused in the worktree, and the
report's `surfaced` entries name the file, layer, worktree path, why it is risky, and a
suggested resolution. Resolve there, `git add`, `git rebase --continue`, then re-invoke the
agent — finished layers are skipped, recorded resolutions replay via rerere.
