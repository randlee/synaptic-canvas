---
description: Launch Claude Code (sonnet) in a supported terminal at a specified directory
argument-hint: "[dir] [--terminal <name>] [--tab] [--tmux] [--identity <name>] [-- <claude args...>]"
allowed-tools: Bash, AskUserQuestion
---

Launch Claude Code with `claude --model sonnet --dangerously-skip-permissions` in a supported terminal.
...
- `--tmux` optional tmux session flow using the session name `"{folder} - sonnet"`.
...
```
Usage: /sc:sonnet [dir] [--terminal <name>] [--tab] [--tmux] [--identity <name>] [-- <claude args...>]

Launch Claude Code (sonnet) in a supported terminal.

Examples:
  /sc:sonnet
  /sc:sonnet ~/projects/foo
  /sc:sonnet ~/projects/foo --terminal ghostty
  /sc:sonnet ~/projects/foo --tab
  /sc:sonnet ~/projects/foo --tmux
  /sc:sonnet ~/projects/foo --identity alice
  /sc:sonnet ~/projects/foo -- --continue
  /sc:sonnet ~/projects/foo --terminal wezterm -- --continue --resume <session-id>
```
...
**Step 6** — Build `SESSION_NAME = "{basename(dir)} - sonnet"`.
