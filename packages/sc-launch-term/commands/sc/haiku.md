---
description: Launch Claude Code (haiku) in a supported terminal at a specified directory
argument-hint: "[dir] [--terminal <name>] [--tab] [--tmux] [--identity <name>] [-- <claude args...>]"
allowed-tools: Bash, AskUserQuestion
---

Launch Claude Code with `claude --model haiku --dangerously-skip-permissions` in a supported terminal.
...
- `--tmux` optional tmux session flow using the session name `"{folder} - haiku"`.
...
- When `--tmux` is used, the launcher also adds `--teammate-mode tmux` to match the current local `haiku` wrapper behavior.
- If `ATM_TEAM` is set and `--identity` is supplied, the launcher registers the member first with `atm teams add-member <team> <identity> --model haiku --cwd <dir>`.
...
```
Usage: /sc:haiku [dir] [--terminal <name>] [--tab] [--tmux] [--identity <name>] [-- <claude args...>]

Launch Claude Code (haiku) in a supported terminal.

Examples:
  /sc:haiku
  /sc:haiku ~/projects/foo
  /sc:haiku ~/projects/foo --terminal wezterm
  /sc:haiku ~/projects/foo --tab
  /sc:haiku ~/projects/foo --tmux
  /sc:haiku ~/projects/foo --identity alice
  /sc:haiku ~/projects/foo -- --continue
  /sc:haiku ~/projects/foo --terminal ghostty -- --continue --resume <session-id>
```
...
```bash
python3 .claude/scripts/sc-term-launch.py launch-claude-model haiku "<resolved_dir>" <TERM_ARG> <TAB_ARG> <IDENTITY_ARG> <CLAUDE_ARG_TAIL>
```

### With `--tmux`

**Step 6** — Build `SESSION_NAME = "{basename(dir)} - haiku"`.
...
```bash
python3 .claude/scripts/sc-term-launch.py launch-claude-model haiku "<resolved_dir>" <TERM_ARG> <TAB_ARG> <IDENTITY_ARG> --tmux "<SESSION_NAME>" <CLAUDE_ARG_TAIL>
```
...
**Connect + Claude pane**:
```bash
python3 .claude/scripts/sc-term-launch.py attach-pane-claude-model "<SESSION_NAME>" haiku --cwd "<resolved_dir>" <TERM_ARG> <TAB_ARG> <IDENTITY_ARG> <CLAUDE_ARG_TAIL>
```

**New session (`next_name`)**:
```bash
python3 .claude/scripts/sc-term-launch.py launch-claude-model haiku "<resolved_dir>" <TERM_ARG> <TAB_ARG> <IDENTITY_ARG> --tmux "<next_name>" <CLAUDE_ARG_TAIL>
```

**Enter custom name** — ask for the session name in a follow-up message, then:
```bash
python3 .claude/scripts/sc-term-launch.py launch-claude-model haiku "<resolved_dir>" <TERM_ARG> <TAB_ARG> <IDENTITY_ARG> --tmux "<custom_name>" <CLAUDE_ARG_TAIL>
```
