...
def test_plugin_skill_paths_match_manifest() -> None:
    manifest = _load_manifest()
    plugin = _load_plugin()
    assert plugin["skills"] == ["./skills/beads-export/SKILL.md"]
    manifest_skills = [
        rel_path
        for rel_path in manifest["artifacts"]["skills"]
        if rel_path.endswith("/SKILL.md")
    ]
    assert manifest_skills == ["skills/beads-export/SKILL.md"]
...
def test_manifest_declares_required_runtime_dependencies() -> None:
    manifest = _load_manifest()
