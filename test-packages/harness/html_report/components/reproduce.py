...
class ReproduceBuilder(BaseBuilder[ReproduceDisplayModel]):
    ...
    def build(self, data: ReproduceDisplayModel) -> str:
        ...
        # Use single-line pre content to avoid whitespace issues
        setup_html = (
            '<div class="reproduce-step">'
            '<div class="reproduce-step-header">'
            '<span class="reproduce-step-label">1. Setup</span>'
            f'{setup_copy_btn}'
            '</div>'
            f'<pre id="setup-commands-{n}">{self.escape(setup_commands)}</pre>'
            '</div>'
        )
        ...
        test_html = (
            ...
            f'{test_copy_btn}'
            '</div>'
            f'<pre id="test-command-{n}">{self.escape(data.test_command)}</pre>'
            '</div>'
        )
        ...
        if data.cleanup_command:
            cleanup_html = (
                ...
                '</div>'
                f'<pre>{self.escape(data.cleanup_command)}</pre>'
                '</div>'
            )
